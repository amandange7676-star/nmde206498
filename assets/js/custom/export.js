...
const SEOData = {
    ...
    if (globalHeader.indexOf("header")!=0 || globalFooter.indexOf("footer")!=0) {
        showCustomAlertBox('error', 'You forgot to add Header or Footer for your website so please add before proceeding further.');
        console.log('You forgot to add Header or Footer for your website so please add before proceeding further.');
        return;
    }
    ...
    headerPages.forEach(pageName => {
        ...
        if (allSectionsOfPage.length === 0) {
            showCustomAlertBox('error', `No sections selected for export for ${pageName}`);
            console.log(`No sections selected for export for ${pageName}`);
            return;
        }
...
function openPreview(){
    ...
    var clientName = getCookie('clientName');
    var clientProjectName = getCookie('projectName');
    var previewMode = getCookie('previewMode') || 'desktop'; // 🔹 ADDED
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            newTab.onload = function () {
                ...
                /* 🔹 MOBILE PREVIEW ONLY (ADDED, NOTHING ELSE TOUCHED) */
                if (previewMode === 'mobile') {
                    ...
                    iframe.contentWindow.document.addEventListener('DOMContentLoaded', function () {
                        ...
                        style.innerHTML = `
                            /* 🔹 ABSOLUTE horizontal scroll kill */
                            html, body {
                                max-width: 100%;
                ...
                appendElement(
                    'script',
                    { src: 'assets/js/custom/editmode.js', type: 'text/javascript' },
                    true
                );
            ...
            // On export button click
                $('#export-btn').off('click').on('click', function () {
                    ...
                    if (!clientName || !projectName) {
                        showCustomAlertBox('error', 'Client and Project details missing. Please fill them first.');
                        console.log("Client and Project details missing. Please fill them first.");
                        return;
                    }
